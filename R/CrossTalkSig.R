...
CrossTalkSig <- function(clu_pairs = NULL, per_num = 1000, pvalue = 0.05) {
    ...
    for (k in 1:per_num) {
        ...
        for (i in 1:nrow(clu_pair)) {
            ...
            # match ligand
            if (is.numeric(ndata1)) {
                ligand_info <- data.frame(gene = names(ndata1), pct = 1, exp_avg = as.numeric(ndata1),
                                          stringsAsFactors = F)
            }
            if (!is.numeric(ndata1)) {
            ...
            if (nrow(ligand_info) > 0) {
                ...
                res_pairs1 <- res_pairs1[, -c(13:16)]
                res_pairs1 <- res_pairs1[, c(2, 1, 3:12)]
            }
            # match receptor
            if (is.numeric(ndata2)) {
                receptor_info <- data.frame(gene = names(ndata2), pct = 1, exp_avg = as.numeric(ndata2),
                                            stringsAsFactors = F)
            }
            if (!is.numeric(ndata2)) {
            ...
            if (nrow(receptor_info) > 0) {
                ...
                res_pairs1 <- res_pairs1[, -c(13:16)]
                res_pairs1 <- res_pairs1[, c(2, 3, 1, 4:12)]
            }
